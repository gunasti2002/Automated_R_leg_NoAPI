...
def load_prover(cfg: PVGConfig, base_model_name: str) -> ProverState:
    ...
    ref_model = AutoModelForCausalLM.from_pretrained(base_model_name, dtype=dtype).to(device)
    ...
    return ProverState(model=model, ref_model=ref_model, tokenizer=tokenizer,
...
def train_prover_role(
    ...
    for item in prompts_and_records:
        ...
        rewards_t = torch.tensor(rewards, device=state.device)
        ...
        for completion, advantage in zip(completions, advantages):
            ...
            with torch.no_grad():
                ref_logprob = compute_sequence_logprob(state.ref_model, state.tokenizer, prompt, completion, state.device)
