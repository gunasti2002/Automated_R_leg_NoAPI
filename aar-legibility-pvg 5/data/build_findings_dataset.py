...
def check_internal_consistency(record: ExperimentRecord, summary: str) -> Tuple[bool, List[str]]:
    ...
    def _normalize(s: str) -> str:
        # ".42" and "0.42" should compare equal; strip a "%" suffix too.
        s = s.rstrip("%")
    ...
    for v in metrics.values():
        if isinstance(v, (int, float)):
            if isinstance(v, float):
                metric_values.add(f"{v:.2f}")
                if 0 <= v <= 1:
                    metric_values.add(f"{v * 100:.0f}%")
            else:
                metric_values.add(str(v))

    normalized_metric_values = {_normalize(mv) for mv in metric_values if mv}
    unmatched = [n for n in numbers_in_summary if _normalize(n) not in normalized_metric_values]
    if unmatched:
        issues.append(
